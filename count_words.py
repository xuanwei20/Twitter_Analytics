...
class MLAnalyzeData:
    ...
    def prepare_twitt_matrix(self, dict):
        with open('disaster_words.txt') as file:
            raw_content = file.read()
            contents_1 = raw_content.lower().split()
        ...
        filename = 'mongodb.csv'
        with open(filename, 'w') as file_object:
            file_object.write(f"tweet number, number of disaster words, number of relevant words\n")
        ...
        for key, value in dict.items():
            ...
            with open(filename, 'a') as file_output:
                file_output.write(f"{n}, {i}, {j}\n")

    def load_mongo_data_matrix(self):
        # Import dataset
        dataset = pd.read_csv('mongodb.csv')
        x = dataset.iloc[:, [1, 2]].values
        return x

    def give_lable(self, matrix):
        y = self.analyze_data(matrix)
        print(y)

    def analyze_mongo_data(self, dict):
        self.prepare_twitt_matrix(dict)
        matrix = self.load_mongo_data_matrix()
        self.give_lable(matrix)
